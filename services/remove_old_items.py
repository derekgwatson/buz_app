...
def delete_deprecated_items_request(request, output_file="output.xlsx",
                                    json_file="../static/buz-app-439103-b6ae046c4723.json"):
    """
    Extracts data from the request and delegates processing.

    Args:
        request: The HTTP request object containing the uploaded file.
        output_file (str): Path to save the processed workbook.
        json_file (str): Path to the Google Sheets credentials JSON file.
    ...
    if not inventory_file:
        raise ValueError("No file uploaded in the request.")

    delete_deprecated_items(
        OpenPyXLFileHandler(file=request.files.get('inventory_file')),
        GoogleSheetsService(json_file=json_file),
        config_manager.config.get("unleashed_data_extract_spreadsheet_id"),
        config_manager.config.get("unleashed_data_extract_range"),
        output_file,
    )
