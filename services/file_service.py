...
def process_backorder_file(file, sheets_service, config, uploads_dir):
    ...
    file_handler = OpenPyXLFileHandler.from_file_like(file=file)
    spreadsheet_id = config['backorder_spreadsheet_id']
    range_name = config['backorder_spreadsheet_range']

    file_handler.save_workbook(original_filename)
    file_handler.save_workbook(upload_filename)  # Example save logic
